...
st.title("Welcome to MarketTrend AI")
st.write("A cutting-edge platform that leverages artificial intelligence to provide real-time stock insights and trends.")
logtxtbox = st.empty()
logtxt = '09:30:00'
logtxtbox.caption(logtxt)

dji_tickers=["3M: MMM", "American Express: AXP", "Apple: AAPL", "Boeing: BA",
"Caterpillar: CAT", "Chevron: CVX", "Cisco Systems: CSCO", "Coca-Cola: KO",
"Dow Inc.: DOW", "ExxonMobil: XOM", "Goldman Sachs: GS", "Home Depot: HD",
"Honeywell: HON", "IBM: IBM", "Intel: INTC", "Johnson & Johnson: JNJ",
"JPMorgan Chase: JPM", "McDonald's: MCD", "Merck: MRK", "Microsoft: MSFT",
"Nike: NKE", "Pfizer: PFE", "Procter & Gamble: PG", "Raytheon Technologies: RTX",
"Salesforce: CRM", "The Walt Disney Company: DIS", "The Travelers Companies: TRV",
"UnitedHealth Group: UNH", "Verizon Communications: VZ", "Visa: V"]

st.selectbox("Stock to chose from",dji_tickers)
stock_symbol = st.text_input("Which stock to analyze?")
...
def process_stock_update(stock_symbol):
    ...
    if not data.empty and not dow_data.empty:
        ...
        calculate_insights(stock_symbol,rolling_window, dow_rolling_window)
...
def calculate_insights(stock_symbol,window, dow_window):
    if len(window) >= 5:
        ...
        if int(market_open_duration) % 5 == 0:  
            get_natural_language_insights(
                stock_symbol,rolling_avg, ema, rsi, bollinger_upper, bollinger_lower,
...
def get_natural_language_insights(
    ...
    prompt = f"""
    You are a professional stock broker. {stock_symbol} stock has a 5-minute rolling average of {rolling_avg:.2f}.
...
if stock_symbol:
    schedule.every(10).seconds.do(lambda: process_stock_update(stock_symbol))  


    message = st.chat_message("assistant")
    message.write(f"Starting real-time simulation for {stock_symbol} stock updates. First update will be processed in 5 minutes...")    
